# Plugin Catalog

_Generated 2026-09-19 — do not edit by hand_

## _universal

| name | tier | url |
|---|---|---|
| superpowers | oss | https://github.com/obra/superpowers |
| karpathy-skills | oss | https://github.com/multica-ai/andrej-karpathy-skills |
| mattpocock-skills | oss | https://github.com/mattpocock/skills |
| prompts-chat | oss | https://github.com/f/prompts.chat |
| ecc | oss | https://github.com/affaan-m/ecc |

## backend

| name | tier | url |
|---|---|---|
| supabase-agent-skills | oss | https://github.com/supabase/agent-skills |
| n8n | oss | https://github.com/n8n-io/n8n |

## cloud

| name | tier | url |
|---|---|---|
| azure-skills | oss | https://github.com/microsoft/azure-skills |
| claude-gcp | oss | https://github.com/shamis6ali/claude-gcp |

## frontend

| name | tier | url |
|---|---|---|
| ui-ux-pro-max | oss | https://github.com/nextlevelbuilder/ui-ux-pro-max-skill |
| gsap-skills | oss | https://github.com/greensock/gsap-skills |

## fullstack

| name | tier | url |
|---|---|---|
| anthropic-doc-skills | official | https://github.com/anthropics/skills |

## mobile

| name | tier | url |
|---|---|---|
| expo-skills | oss | https://github.com/expo/skills |

## salesforce

| name | tier | url |
|---|---|---|
| sf-skills | oss | https://github.com/forcedotcom/sf-skills |
